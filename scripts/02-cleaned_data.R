...
business_bankruptcies <- 
  read.csv("Number of Business Bankruptcies for the City of Toronto.csv")

# Convert Period to a date format that includes the year and month
food_bank_usage$Date <- 
  as.Date(paste(food_bank_usage$Year, food_bank_usage$Period, "1", sep="-"), format="%Y-%b-%d")
business_bankruptcies$Date <- 
  as.Date(paste(business_bankruptcies$Year, business_bankruptcies$Period, "1", sep="-"), format="%Y-%b-%d")
...
#Write the cleaned output data to parquet
write_parquet(merged_data, "inputs/clean_data.parquet")
# Write the cleaned output data to csv as well
